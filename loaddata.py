...
# low price and high price to remove outliers
t_low_price = 2.0
t_high_price = 500.0
...
from_timestamp = lambda x: datetime.fromtimestamp(int(x) / 1000.)


def get_uni(start, end, lookback, uni_size=1400):
    # get the universe list as combination of symbols and sector names
    ...
    univ_df = pd.read_csv(univ_file, header=0, usecols=['sid', 'ticker_root', 'status', 'country', 'currency'],
                          index_col=['sid'])
    print("Universe size (raw): {}".format(len(univ_df.index)))
    # contains price data for given time frame in US
    ...
    # company data
    secdata_df = pd.read_csv(secdata_file, header=0, compression='gzip',
                             usecols=['sid', 'symbol', 'sector', 'ind', 'group', 'sector_name', 'ind_name',
                                      'estu_inter', 'estu_barra4s'], index_col=['sid'])
    ...
    price_df = pd.read_csv(price_file, header=0,
                           usecols=['sid', 'ticker', 'close', 'tradable_med_volume_21', 'mkt_cap'], index_col=['sid'])
...
def load_barra(uni_df, start, end, cols=None):
    ...
    while date < end:
        ...
        try:
            ...
            print
            "File not found: {}".format(barra_file)
            date += timedelta(days=1)
            continue
...
def load_daybars(uni_df, start, end, cols=None, freq='30Min'):
    ...
    while (date < end):
        ...
        print
        "Loading {}".format(bar_file)
        try:
            bars_df = pd.read_csv(bar_file, compression='gzip', sep="|", header=None,
                                  names=['ticker', 'iclose_ts', 'dopen', 'dhigh', 'dlow', 'iclose', 'qhigh', 'qlow',
                                         'dvwap', 'dvolume', 'dtrades', 'open_c', 'open_c_volume', 'close_c',
                                         'close_c_volume'], index_col=['iclose_ts'],
                                  converters={'iclose_ts': from_timestamp}, usecols=cols)
        except IOError:
            print
            "File not found: {}".format(bar_file)
            date += timedelta(days=1)
            continue
        ...
        bars_df = pd.merge(uni_df[uni_df['date'] == date], bars_df, how='inner', left_on=['date', 'ticker'],
                           right_on=['date', 'ticker'], sort=True, suffixes=['', '_dead'])
...
def load_prices(uni_df, start, end, cols):
    if cols is None:
        cols = ['open', 'high', 'low', 'prim_volume', 'comp_volume', 'shares_out', 'ret', 'log_ret',
                'prim_med_volume_21', 'prim_med_volume_60', 'comp_med_volume_21', 'comp_med_volume_60',
                'tradable_med_volume_60', 'volat_21', 'volat_60', 'cum_log_ret', 'overnight_log_ret', 'today_log_ret',
                'overnight_volat_21', 'today_volat_21', 'split', 'div']
    ...
    while (date < end):
        ...
        try:
            ...
            print
            "File not found: {}".format(price_file)
            date += timedelta(days=1)
            continue

        prices_df['date'] = prices_df['gdate'] = date
        #        print uni_df.head()
        prices_df = pd.merge(uni_df.reset_index(), prices_df.reset_index(), how='inner', left_on=['sid'],
                             right_on=['sid'], sort=True, suffixes=['', '_dead'])
    ...
    result_df['expandable'] = (result_df['close_y'] > e_low_price) & (result_df['close_y'] < e_high_price) & (
            result_df['mdvp_y'] > e_min_advp)
...
def load_volume_profile(uni_df, start, end, freq='30Min'):
    ...
    while (date < end):
        ...
        print
        "Loading {}".format(volume_file)
        try:
            volume_df = pd.read_csv(volume_file, header=0, index_col=['sid'])
        except IOError:
            print
            "File not found: {}".format(volume_file)
            date += timedelta(days=1)
            continue
        ...
        volume_df = volume_df[
            (volume_df['level_1'] != 'med_open_volume') & (volume_df['level_1'] != 'med_close_volume') & (
        ...
        print
        "merging..."
        volume_df = pd.merge(uni_df, volume_df, how='inner', left_index=True, right_index=True, sort=True,
                             suffixes=['', '_dead'])
...
def load_bars(uni_df, start, end, cols=None, freq=30):
    ...
    while (date < end):
        ...
        try:
            bars_df = pd.read_csv(bar_file, compression='gzip', sep="|", header=None,
                                  names=['ticker', 'bopen_ts', 'iclose_ts', 'bopen', 'bfirst', 'bhigh', 'blow', 'blast',
                                         'iclose', 'bvwap', 'bvolume', 'btrades', 'meanSpread', 'meanEffectiveSpread',
                                         'meanBidSize', 'meanAskSize', 'bidHitTrades', 'midHitTrades', 'askHitTrades',
                                         'bidHitDollars', 'midHitDollars', 'askHitDollars', 'outsideTrades',
                                         'outsideDollars'],
                                  converters={'iclose_ts': from_timestamp, 'bopen_ts': from_timestamp},
                                  na_values=['-1'])
        except IOError:
            print
            "File not found: {}".format(bar_file)
        ...
        bars_df = pd.merge(uni_df[uni_df['date'] == date], bars_df, how='inner', left_on=['ticker'],
                           right_on=['ticker'], sort=True, suffixes=['', '_dead'])
        result_dfs.append(bars_df)
        date += timedelta(days=1)
...
def load_earnings_dates(uni_df, start, end):
    ...
    while (date < end):
        ...
        try:
            ...
            print
            "File not found: {}".format(earnings_file)
            date += timedelta(days=1)
            continue

        df['date'] = date
        df = pd.merge(uni_df[uni_df['date'] == date], df, how='inner', left_on=['sid'], right_on=['sid'], sort=True,
                      suffixes=['', '_dead'])
    ...
    result_df.ix[result_df['earn_rpt_time_norm'] == "AFTMKT", 'daysToEarn'] = result_df.ix[result_df[
                                                                                               'earn_rpt_time_norm'] == "AFTMKT", 'daysToEarn'] + 1
    result_df.set_index(keys=['date', 'sid'], inplace=True)
    result_df = remove_dup_cols(result_df)
    return result_df
...
def load_locates(uni_df, start, end):
    ...
    while (date < end):
        ...
        try:
            ...
            print
            "File not found: {}".format(locates_file)
            date += timedelta(days=1)
            continue
...
def load_qb_implied_orders(mdir, start, end):
    ...
    for ff in files:
        ...
        "Loading {} for {}".format(ff, fdate)
        df = pd.read_csv(ff, header=0, parse_dates=True, sep=",",
                         usecols=['time', 'sid', 'ref_price', 'net_qty', 'open_long_amt', 'open_short_amt'],
                         index=['time', 'sid'])

        df['open_order'] = (df['open_long_amt'] - df['open_short_amt']) / df['ref_price']
        result_l = list()
        for timeslice in ['10:02', '10:32', '11:02', '11:32', '12:02', '12:32', '13:02', '13:32', '14:02', '14:32',
                          '15:02', '15:32']:
...
def load_qb_eods(mdir, start, end):
    ...
    for ff in files:
        ...
        "Loading {} for {}".format(ff, fdate)
        df = pd.read_csv(ff, header=0, parse_dates=True, sep=",",
                         usecols=['time', 'sid', 'ref_price', 'today_long', 'today_short'])
...
def load_ratings_hist(uni_df, start, end, intra=False):
    if intra:
        times = ['09:45', '10:00', '10:15', '10:30', '10:45', '11:00', '11:15', '11:30', '11:45', '12:00', '12:15',
    ...
    while (date < end):
        ...
        for time in times:
            ...
            sql = "select * from t_ibes_hist_rec_snapshot where timestamp between '{} {}' and '{} {}' group by sid, ibes_ticker, estimator having timestamp = max(timestamp)".format(
                startDateStr, time, endDateStr, time)
            print
            sql
    ...
    # consensus
    result_df = df.groupby(level=[tstype, 'sid']).agg(
        {'ibes_rec_code': [np.mean, np.median, np.std, 'count', np.max, np.min], 'timestamp': 'last'})
...
def load_target_hist(uni_df, start, end, intra=False):
    ...
    while (date < end):
        ...
        for time in times:
            # timeAdjusted = str(int(time.split(":")[0]) - 3) + ":" + time.split(":")[1]
            sql = "select * from t_ibes_hist_ptg_snapshot where timestamp between '{} {}' and '{} {}' and horizon in ('', 12) and value > 0 group by sid, ibes_ticker, estimator having timestamp = max(timestamp)".format(
                startDateStr, time, endDateStr, time)
            print
            sql
            df = psql.frame_query(sql, con)
    ...
    # consensus
    result_df = df.groupby(level=['date', 'sid']).agg(
        {'value': [np.mean, np.median, np.std, 'count', np.max, np.min], 'timestamp': 'last'})
    result_df.columns = result_df.columns.droplevel(0)
    for column in result_df.columns:
...
def load_estimate_hist(uni_df, start, end, estimate):
    ...
    while (date < end):
        ...
        for time in times:
            ...
            sql = "select * from t_ibes_det_snapshot where timestamp between '{} {}' and '{} {}' and measure = '{}' and forecast_period_ind = 1 and forecast_period_end_date > {} and forecast_period_end_date < {} group by sid, ibes_ticker, estimator, forecast_period_ind, forecast_period_end_date having timestamp = max(timestamp) order by sid, forecast_period_end_date;".format(
                startDateStr, time, endDateStr, time, estimate, minPeriod, maxPeriod)
            print
            sql
            df = psql.frame_query(sql, con)
            df['value'] = df['value'].astype(str)
...
def load_consensus(dtype, uni_df, start, end, freq='30Min'):
    ...
    for name, group in sid_groups:
        resampled_df = group.reset_index(level=0).resample(freq, how='last', fill_method='pad', closed='left',
                                                           label='right')
